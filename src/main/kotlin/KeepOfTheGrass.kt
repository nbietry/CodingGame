...
class Cell(
    val x: Int, val y: Int,
    val scrapAmount: Int, owner: Int,
    val units: Int, val recycler: Int, canBuild: Int, canSpawn: Int,
...
class Robot(
    val x: Int, val y: Int, var target: Cell?
){
    override fun toString(): String {
...
fun main(args : Array<String>) {
    ...
    while (true) {
        ...
        run manageRobots@{
            for (robot in game.myRobots) {
                val target: Cell? = findClosestFreeCell(robot, game.freeCells)
                if (target != null) {
                    val amount = target.units
                    if (robot.target == game.battleground[robot.y][robot.x]) {
                        robot.target = target
                        actions.add(
